...
from leap_motion_controller.msg import Set
...
def callback(data):
	pinch = "servo_gripper command is %s" % data.data
	rospy.loginfo(pinch)
	
	desired_angle = (open_angle + (close_angle-open_angle)*float(data.data))
...
def listener_talker():
    ...
    rospy.Subscriber("stt/spoken_text", String, callback)
